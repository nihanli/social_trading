...
with tab_suggest:
    ...
    if "exits_time_stop" in recent.columns and "total_trades" in recent.columns:
        avg_total = recent["total_trades"].mean()
        avg_ts = recent["exits_time_stop"].mean()
        if pd.notna(avg_total) and avg_total > 0 and pd.notna(avg_ts) and avg_ts / avg_total > 0.5:
            suggestions.append({
    ...
    if "exits_atr_stop" in recent.columns and "total_trades" in recent.columns:
        avg_atr_exits = recent["exits_atr_stop"].mean()
        if pd.notna(avg_total) and avg_total > 0 and pd.notna(avg_atr_exits) and avg_atr_exits / avg_total > 0.4:
            suggestions.append({
    ...
    if "exits_sentiment_reversal" in recent.columns and "total_trades" in recent.columns:
...
with tab_grid:
    ...
    days_grid = st.slider("Use last N days of trades", 7, 90, 30, key="grid_days")
    ...
    if st.button("Run Grid Search", type="primary") and combos:
        results = []
        ...
        for i, (tp, atr_mult) in enumerate(combos):
            ...
            for _, row in trades_df.iterrows():
                ...
                atr_approx = ep * 0.02  # ~2% ATR proxy
                if direction == "LONG":
                    stop  = ep * (1 - atr_mult * atr_approx / ep)
                    target = ep * (1 + tp)
                else:
                    stop  = ep * (1 + atr_mult * atr_approx / ep)
                    target = ep * (1 - tp)

                exit_p = float(row["exit_price"] or ep)
                qty = float(row["shares"])
                cost_bps = 0.002  # 20 bps round-trip

                if direction == "LONG":
                    raw_pnl = (min(max(exit_p, stop), target) - ep) * qty
                else:
                    raw_pnl = (ep - min(max(exit_p, target), stop)) * qty
        ...
        if results:
            ...
            if st.button("Apply best settings to config", type="primary"):
